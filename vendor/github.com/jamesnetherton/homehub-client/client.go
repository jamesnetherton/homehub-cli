...
func (c *client) doReboot(xpath string) (err error) {
	req := newRebootRequest(&c.authData, xpath)
	_, err = req.send()
	return err
}

func (c *client) getBandwidthUsage(xpath string) (result string, err error) {
	bandwidthMonitorRequest := newBandwidthMonitorRequest(&c.authData, xpath)
...
func (c *client) getEventLog(xpath string) (result string, err error) {
	eventLogRequest := newEventLogRequest(&c.authData, xpath)
